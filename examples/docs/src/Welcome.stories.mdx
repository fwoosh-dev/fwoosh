...
```ts fwoosh.config.mjs focus=2,6
import type { FwooshOptions } from "fwoosh";
...
If you're using typescript you can define the types of your `Meta` and `Story`s.
With this you will get autocompletion for decorators and other configuration.

```ts fwoosh.config.mjs focus=3:6,13:16
import type { FwooshOptions } from "fwoosh";
import ReactPlugin from "@fwoosh/react";
import type {
