...
export class PageBuilder {
  ...
  constructor(options: PageBuilderOptions, layouts: Layout[]) {
    this.options = options;
    this.layouts = layouts;
  }

  private async build(esBuildOptions: esbuild.BuildOptions) {
  ...
  async buildPage(pages: string[], watch = false): Promise<PageBuild> {
    ...
    const moveFilesToOut = async (outdir: string) => {
      ...
      await Promise.all(
        virtualClientPages.map(async (page) => {
          ...
          await fs.copy(clientJs, path.join(this.options.outDir, filePath));
        })
      );
    ...
    try {
      ...
      const builder = await this.build({
        outdir,
  ...
  async buildPages() {
    ...
    return this.buildPage(pages);
  }

  watchPages(options: WatchPagesOptions = { port: 3000 }) {
    return new Promise<void>((resolve, reject) => {
      ...
      chokidar
        ...
        .on("change", async (changePath) => {
          const layout = this.layouts.find((l) => changePath.includes(l.path));
